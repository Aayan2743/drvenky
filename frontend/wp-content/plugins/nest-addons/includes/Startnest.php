...
class Startnest{
      ...
      */
      public function nest_enqueue_scripts(){
        ...
        $show_add_to_cart = isset( $nest_theme_mod['show_add_to_cart'] ) ? $nest_theme_mod['show_add_to_cart'] : '';
        ...
        wp_enqueue_style('style', get_template_directory_uri().'/style.css' , array('elementor-frontend'  , 'select2css' , 'nest-wooproductfilter') , time() , 'all'); 
        ...
        if($show_add_to_cart == true):
          wp_enqueue_style('nest-showcart', NEST_ADDONS_URL .'assets/css/showcart.css' );   
        endif; 
        if($catalogmode == true  ||  (is_singular(array('product')) && get_post_meta(get_the_ID() , 'catalog_mode_for_this', true) == true) ):
          wp_enqueue_style('nest-cate', NEST_ADDONS_URL .'assets/css/catalog.css' );   
        endif;
