...
class Nest_Category_Filter_Widget extends WP_Widget {
    ...
    function render_category_filter() {
        ...
        $current_category_filters = isset($_GET['category']) ? explode(',', urldecode($_GET['category'])) : array();
...
function custom_product_category_filter($query) {
    if (!is_admin() && $query->is_main_query()) {
        if (isset($_GET['category'])) {
            $categories_encoded = $_GET['category'];
            // Decode the category parameter
            $categories = explode(',', urldecode($categories_encoded));
            $tax_query = array(
                array(
                    'taxonomy' => 'product_cat',
                    'field' => 'slug',
