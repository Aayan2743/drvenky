...
export default {
    ...
    computed: {
        ...
        formattedPhone: {
        get() {
            return `+91${this.props.form.phone || ''}`;
        },
        set(value) {
            this.props.form.phone = value.replace(/^\+91/, ''); // Remove +91 if already there
        }
         }
    },
    methods: {
    ...
    try {
        ...
        // Dispatch save action to Vuex store
        this.$store.dispatch("frontendAddress/save", { 
            form: this.$props.props.form, 
            temp_id: tempId 
        }).then((response) => {
            // On success, reset the form and close the modal
            this.reset();
            this.loading.isActive = false;
            alertService.success(this.$t("alert.address_saved_successfully"));
        }).catch((error) => {
            // Handle validation errors or other failures
            if (error.response && error.response.data.errors) {
                this.errors = error.response.data.errors;
            } else {
                alertService.error(this.$t("alert.unexpected_error"));
            }
            this.loading.isActive = false;
        });
    } catch (err) {
