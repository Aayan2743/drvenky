<template>
    ...
    <!-- Delivery Radio Button checkout-delivery" -->
    <input 
        @change="changeOrderType(orderTypeEnum.DELIVERY)" 
        id=orderTypeEnum.DELIVERY
        ...
        name="orderType">
    <label 
        for=orderTypeEnum.DELIVERY 
        class="py-1.5 px-3.5 rounded-2xl text-sm font-semibold capitalize transition cursor-pointer">
        {{ $t('label.delivery') }}
    </label>
</div>

<div class="relative cursor-pointer">
    <!-- Pick Up Radio Button checkout-pickup -->
    <input 
        @change="changeOrderType(orderTypeEnum.PICK_UP)" 
        id=orderTypeEnum.PICK_UP 
    ...
    <label 
        ...
        <div class="col-12 lg:col-4">
            <CouponComponent/>
            <!-- <SummeryComponent :isTelangana="isTelangana" /> -->
            <SummeryComponent  />
...
export default {
    ...
    computed: {
        ...
        // console.log("Checking State:", address?.state);
        // return address?.state === "Telangana";
        ...
        // getShippingAddress() {
        //     return this.$store.state.shippingAddress; // Example: If using Vuex
        // },
        isTelangana() {
            const address = this.getShippingAddress;
            console.log("Checking State:", address?.state);
        ...
        getShippingAddress: function () {
            ...
            // const address = this.$store.getters['frontendCart/shippingAddress'];
            // console.log("Fetched Shipping Address:", address);
            // return address;

            const address = this.$store.getters['frontendCart/shippingAddress'] || {};
                console.log("Shipping Address:", address.state);
                return address;

            // console.log("ship",this.$store.getters['frontendCart/shippingAddress']);
    ...
    mounted() {
        ...
        console.log(this.getShippingAddress?.state?.toLowerCase());
    ...
    methods: {
        changeOrderType: function (e) {
            ...
            console.log("Dispatched Order Type:", e); // Debug to verify the value
    this.$store.dispatch('frontendCart/updateOrderType', e)
        .then(() => {
            console.log("Order type updated successfully in Vuex.");
        })
        .catch((error) => {
