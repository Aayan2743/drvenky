                    ...
                    <div class="row">
                        ...
                        <div class="col-12 sm:col-6 md:col-4 xl:col-3">
                            ...
                            <vue-select class="db-field-control f-b-custom-select" id="user_id"
                                v-model="props.search.user_id" :options="customers_data" label-by="email" value-by="id"
                ...
                <table class="db-table stripe" id="print">
                    <thead class="db-table-head">
                        <tr class="db-table-head-tr">
                            ...
                            <th class="db-table-head-th">Store</th>
                            <th class="db-table-head-th">Payment Type</th>
                    ...
                    <tbody class="db-table-body" v-if="orders.length > 0">
                        <tr class="db-table-body-tr" v-for="order in orders" :key="order">
                            ...
                            <td class="db-table-body-td">{{ order.user_data.email}}</td>
                            <td class="db-table-body-td">{{ order.payment_method}}</td>
...
export default {
    ...
    data() {
        ...
        //     customers1: order.user_data.map(user => ({
        //     id: user.id,
        //     email: user.email,
        // })), 
    ...
    computed: {
        orders: function () {
            const customerData = this.$store.getters['posOrder/lists'];

            // console.log("dfsdjkfhsdjkfhsdkjfhjk",customerData );

            const userEmails = customerData.map(item => item.user_data?.email);
         
            const uniqueUserEmails = Array.from(
                new Set(customerData.map(item => item.user_data?.email).filter(email => email))
                );

                // console.log('Asif Data Email:', uniqueUserEmails); 
            
                // console.log('orders :', customerData); 
            return this.$store.getters['posOrder/lists'];

        },

        customers_data: function() {
        const customerData = this.$store.getters['posOrder/lists'];
        // console.log("Asiof" +customerData )
         
        return Array.from(
        new Set(customerData.map(item => item.user_data?.email).filter(email => email))
    ).map(email => {
        // Find the first entry with the email to get the user_data.id
        const user = customerData.find(item => item.user_data?.email === email);
        return {
            id: user?.user_data?.id,  // Use the user_data.id as the id
            email: email               // Use email as the label
        };
    });
        ...
        //                 console.log('Unique User Emails:', uniqueUserEmails);
        //                 console.log('All Orders:', customerData);

        //                 // If you need unique emails for use elsewhere, you can return them as needed:
        //                 return {
        //                     orders: customerData, // Original order data
        //                     uniqueEmails: uniqueUserEmails, // Unique user emails
        //                 };
        //             },
        ...
        customers: function () {
            const customerData = this.$store.getters['user/lists'];
            // console.log('Customer Data:', customerData); 

            return this.$store.getters['user/lists'];
        },
        operators: function () {
            return this.$store.getters['user/lists_operator'];
        },
        pagination: function () {
