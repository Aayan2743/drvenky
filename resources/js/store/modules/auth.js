...
export const auth = {
    ...
    actions: {
            ...
            return axios.post('/auth/send-otp', { phone })
                ...
                .catch(error => {
                    if (error.response.status === 422) {
                        // Return the validation errors to the component
        ...
        // verifyOtp({ commit }, form) {
        //     return axios.post('/auth/verify-otp', form)
        //         .then((response) => {
        ...
        //             context.commit('authLogin', response.data);
        //             resolve(response);
        //         });
        // },
        ...
        verifyOtp: function (context, payload) {
            return new Promise((resolve, reject) => {
                axios.post('/auth/verify-otp', payload)
                    .then((res) => {
                    ...
                    .catch((err) => {
                        if (err.response && err.response.status === 422) {
                            reject(err.response.data.errors); // Pass the validation errors
                        } else {
                            reject(err); // Pass other errors
                        }
                    });
            });
