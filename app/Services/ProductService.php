...
//dd($request->HsnCode);
        try {
            DB::transaction(function () use ($request) {
                $this->product = Product::create($request->validated() + ['slug' => Str::slug($request->name), 'variation_price' => $request->selling_price, 'HsnCode' => $request->HsnCode, ]);
                if ($request->tags) {
    ...
    public function showWithRelation1(Product $product, Request $request)
    {
        ...
        try {
            return Product::with('media', 'videos', 'category', 'unit', 'taxes')
                ...
                ->with(['reviews' => fn ($query) => $query->with('user', 'media')->take($request->get('review_limit', 3))])
                ->withReviewRating()
                ->where(['id' => $product->barcode_id, 'status' => Status::ACTIVE])
                ->first();
        } catch (Exception $exception) {
            Log::info($exception->getMessage());
            throw new Exception($exception->getMessage(), 422);
